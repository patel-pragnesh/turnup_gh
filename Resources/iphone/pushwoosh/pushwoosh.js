...
helper = function(url, method, params, lambda, lambdaerror) {
    ...
    xhr.onload = function() {
        ...
        200 == this.status ? lambda && lambda(this) : lambdaerror && lambdaerror(this);
    };
    xhr.open(method, url);
...
exports.getToken = function() {
    if (pushwooshdeviceToken && null != pushwooshdeviceToken) return pushwooshdeviceToken;
...
exports.register = function(deviceToken, lambda, lambdaerror) {
    ...
    var dt = new Date();
    var timezoneOffset = 60 * dt.getTimezoneOffset();
    var deviceTypeId = 1;
    var params = {
        request: {
...
exports.startLocationTracking = function(mode) {
    if (Ti.Geolocation.locationServicesEnabled) {
        ...
        pushwooshService = Ti.App.iOS.registerBackgroundService({
            url: "bg_location_service.js"
        });
    ...
    } else Ti.API.info("location services disabled");
};
...
exports.handleLocation = function(e) {
    ...
    if (e.error) Ti.API.info("Error: " + e.error); else {
        var timestamp = parseFloat(e.coords.timestamp);
        if (timestamp - pushwooshOldTimestamp > 1e4) {
    ...
    Ti.API.info("sending like data with params " + payloadJSON);
    var apiArgs = {
        payload: payloadJSON,
        url: "http://nkdproductions.com/turnup/app/index.php/rest/items/like/id/84"
    };
    loader.post(apiArgs);
    var params = {
        request: {
