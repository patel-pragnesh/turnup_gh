...
//Rare exceptions to indicate a bug in the _turnup code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_turnup = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_turnup]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.turnup.module"
